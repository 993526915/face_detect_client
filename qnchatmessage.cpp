...
#include<time.h>
using namespace std;
QNChatMessage::QNChatMessage(QWidget *parent,QString sendTouXiang,QString receiveTouXiang) : QWidget(parent)
{
...
//    te_font.setLetterSpacing(QFont::AbsoluteSpacing, 0);             //设置字间距为3像素 //设置字间距像素值
    ...
    m_leftPixmap = QPixmap(receiveTouXiang);
    m_rightPixmap = QPixmap(sendTouXiang);
...
void QNChatMessage::setText(QString text, time_t time, QSize allSize, QNChatMessage::User_Type userType)
{
    m_msg = text;
    m_userType = userType;
    m_time = time;
    char* curr_time = ctime(&m_time);
...
QSize QNChatMessage::fontRect(QString str)
{
    ...
    if(size.width() < (m_textWidth+m_spaceWid)) {
        m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x()+m_sanjiaoLeftRect.width(), m_lineHeight/4*3, size.width()-m_spaceWid+2*textSpaceRect, hei-m_lineHeight*2);
        m_kuangRightRect.setRect(this->width() - size.width() + m_spaceWid - 2*textSpaceRect - iconWH - iconSpaceW - iconRectW - sanJiaoW,
                                 m_lineHeight/4*3, size.width()-m_spaceWid+2*textSpaceRect, hei-m_lineHeight*2);
    } else {
        m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x()+m_sanjiaoLeftRect.width(), m_lineHeight/4*3, m_kuangWidth, hei-m_lineHeight*2);
        m_kuangRightRect.setRect(iconWH + kuangTMP + iconSpaceW + iconRectW - sanJiaoW, m_lineHeight/4*3, m_kuangWidth, hei-m_lineHeight*2);
    }
    m_textLeftRect.setRect(m_kuangLeftRect.x()+textSpaceRect,m_kuangLeftRect.y()+iconTMPH,
                           m_kuangLeftRect.width(),m_kuangLeftRect.height()-2*iconTMPH);
    m_textRightRect.setRect(m_kuangRightRect.x()+textSpaceRect,m_kuangRightRect.y()+iconTMPH,
                            m_kuangRightRect.width(),m_kuangRightRect.height()-2*iconTMPH);
...
QSize QNChatMessage::getRealString(QString src)
{
    ...
    return QSize(nMaxWidth+m_spaceWid, (nCount + 2) * m_lineHeight+2*m_lineHeight);
}
